...
local COL_JAMMER    = Color(255, 190, 50)
...
local haloColor     = Color(255, 60, 40)
local barColor      = Color(255, 190, 50)

local smoothHealth = {}

surface.CreateFont("CommsJammer_Label", {
    font = "Roboto", size = 100, weight = 700, antialias = true,
})
surface.CreateFont("CommsJammer_HP", {
...
function ENT:Draw()
    ...
    -- Label above entity
    local labelPos = pos + Vector(0, 0, 45)
    cam.Start3D2D(labelPos, ang, 0.05)
        local labelText = active and "COMMS JAMMER" or "JAMMER [DISABLED]"
        local labelCol = active and COL_JAMMER or COL_INACTIVE

        if active then
            local pulse = 0.7 + 0.3 * math.abs(math.sin(CurTime() * 2))
            labelCol = Color(255, 190, 50, 180 + 75 * pulse)
        end

        draw.RoundedBox(0, -300, -30, 600, 60, COL_BG)
        draw.SimpleText(labelText, "CommsJammer_Label", 1, 1, COL_SHADOW, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText(labelText, "CommsJammer_Label", 0, 0, labelCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    cam.End3D2D()

    -- Health bar when active
    if active and smooth < 0.99 then
        local barPos = pos + Vector(0, 0, 55)
        cam.Start3D2D(barPos, ang, 0.05)
            ...
            draw.SimpleText(pct, "CommsJammer_HP", 1, barH / 2 + 1, COL_SHADOW, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText(pct, "CommsJammer_HP", 0, barH / 2, COL_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end
...
-- Red pulsing halo when active
hook.Add("PreDrawHalos", "CommsJammer_ActiveHalo", function()
    local ply = LocalPlayer()
    if not IsValid(ply) then return end
    ...
    if #jammers > 0 then
        local pulse = 0.4 + 0.6 * math.abs(math.sin(CurTime() * 2))
